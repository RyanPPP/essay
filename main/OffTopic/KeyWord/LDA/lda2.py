...
#获取当前路径
path = os.getcwd()+'/main/OffTopic/KeyWord/LDA'
...
conf.read(os.getcwd()+"/main/OffTopic/setting.conf")
#文件路径
...
class DataPreProcessing(object):
    ...
    def cachewordidmap(self):#统计出现的单词并写入wordidmap.dat
...
class LDAModel(object):
    ...
    def __init__(self,dpre):
        ...
        self.theta = np.array([ [0.0 for y in xrange(self.K)] for x in xrange(self.dpre.docs_count) ])#主题-文档矩阵
        self.phi = np.array([ [ 0.0 for y in xrange(self.dpre.words_count) ] for x in xrange(self.K)]) #主题-词矩阵
...
def filein():
    with codecs.open('main/OffTopic/KeyWord/LDA/data/model_phi.dat', 'r', 'utf-8') as f:
        docs1 = f.readlines()

        docsline = docs1[0].strip().split()
        array = np.zeros((len(docs1),len(docsline)),dtype="double")

        # print len(array)
        # print len(array[0])
        for x in range(0,len(docs1)):
            if docs1[x] != "":
                tmp = docs1[x].strip().split()
