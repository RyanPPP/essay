...
def getChart2(text):
    ...
    #简易词性表
    posDict,pos_all = loadPosDict(os.getcwd()+'/main/check/posDict_min.txt')
...
def getErrors(text):
    ...
    posDict_detail, pos_all_detail = loadPosDict(os.getcwd() + "/main/check/posDict_min.txt")
    english_punctuations = [',', '.', ':', ';', '?', '(', ')', '[', ']', '&', '!', '*', '@', '#', '$', '%']
    # print matches
...
def getFeedback(text):
    ...
    posDict_detail, pos_all_detail = loadPosDict(os.getcwd() + "/main/check/complextype.txt")
